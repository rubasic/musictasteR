...
plot_cross <- function(year,x_axis=energy,y_axis=danceability){
  year <- enquo(year)
  x_axis <- enquo(x_axis)
  y_axis <- enquo(y_axis)
  x_axis_name <- as.character(x_axis)
  y_axis_name <- as.character(y_axis)
  tracklist <- spotify_track_data %>% filter(year == {!!year} | year == "0" ) %>% select(year,artist_name,track_name,!!x_axis,!!y_axis)
...
plot_cross(2014,energy,valence)
#tests: does this function return 
...
energy <- "energy" #we need to save the dimension as a character
u <- as.character(energy)
#tooltip takes as characters information that is available in the text = field 
ggplotly(plot, tooltip = c("text", "artist",glue("{energy}"),"danceability" ))
