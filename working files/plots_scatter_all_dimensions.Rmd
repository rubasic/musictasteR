...
plot_cross <- function(database,year,x_axis=energy,y_axis=danceability){
  ...
  tracklist <- database %>% filter(year == {!!year} | year == "0" ) %>% select(year,artist_name,track_name,!!x_axis,!!y_axis,new)
  ...
  plot <- ggplot(tracklist, aes(!!x_axis, !!y_axis))  +
    geom_point(aes(text = track_name, artist=artist_name, color=new, size = 0.1),alpha = 1/2) + theme_minimal() + 
    xlim(0,1) + ylim (0,1)
  }
  else{
   plot <- ggplot(tracklist, aes(!!x_axis, !!y_axis))  +
...
plot_cross(spotify_track_data,2010,energy,valence)
